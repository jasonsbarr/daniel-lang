...
export const module = (runtime, err) => {
  ...
  const concat = (...lists) => {
    ...
    for (let list of lists) {
      if (!Array.isArray(list)) {
        return err["exn:arguments"]("concat", "1 or more lists", getType(list));
      }
  ...
  const tail = (list) => {
    if (!Array.isArray(list)) {
      return err["exn:arguments"]("tail", "list", getType(list));
    }
  ...
  const gensym = (prefix = Symbol()) =>
    `_${prefix.description}${hash(v4()).slice(0, 6)}`;
