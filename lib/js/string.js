...
export const module = (runtime, err, io) => {
  ...
  const toList = (str) => {
    checkString(str, "string->list");
    return [...str];
  };

  const upcase = (str) => {
    checkString(str, "upcase");
    return str.toUpperCase();
  };

  const downcase = (str) => {
    checkString(str, "downcase");
    return str.toLowerCase();
  };

  const capitalizeStr = (str) => {
    checkString(str, "capitalize");
    return capitalize(str);
  };

  const titleCase = (str) => {
    checkString(str, "title-case");
    return capitalize(str, { all: true });
  };

  const normalize = (str) => {
    checkString(str, "normalize");
    return str.normalize();
  };

  const normalizeSpaces = (str) => {
    checkString(str, "normalize-spaces");
    return str
      .trim()
      .split(" ")
      .filter((w) => !/^\s*$/.test(w))
